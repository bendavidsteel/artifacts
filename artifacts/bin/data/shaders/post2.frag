...
vec3 pixelSorter() {
    ...
    vec3 lastColour = texture(last2, gl_FragCoord.xy).rgb;
    vec3 lastUpColour = texture(last2, gl_FragCoord.xy + sortDir).rgb;
    vec3 lastDownColour = texture(last2, gl_FragCoord.xy - sortDir).rgb;
