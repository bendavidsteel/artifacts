...
vec3 pixelSorter() {
    ...
    vec3 lastColour = texture(last4, gl_FragCoord.xy).rgb;
    vec3 lastUpColour = texture(last4, gl_FragCoord.xy + sortDir).rgb;
    vec3 lastDownColour = texture(last4, gl_FragCoord.xy - sortDir).rgb;
...
void main(){
    ...
    if (beat == 4) {
        out_color.rgb = texture(temple, coord).rgb;
    }
    out_color.a = 1.;
}